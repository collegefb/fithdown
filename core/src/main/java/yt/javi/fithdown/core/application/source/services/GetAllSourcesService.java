...
public class GetAllSourcesService implements Service<GetAllSourcesRequest, Collection<SourceResponse>> {
  private final SourceRepository sourceRepository;

  private final SourceResponseFactory responseFactory;

  public GetAllSourcesService(SourceRepository sourceRepository, SourceResponseFactory responseFactory) {
  ...
  public Collection<SourceResponse> execute(GetAllSourcesRequest request) {
    return sourceRepository.findAll().stream().map(responseFactory::sourceResponse).collect(toList());
  }
}
