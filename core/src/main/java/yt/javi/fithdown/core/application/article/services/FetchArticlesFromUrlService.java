...
public class FetchArticlesFromUrlService
    implements Service<FetchArticlesFromUrlRequest, Collection<ArticleResponse>> {

  private final ArticleRepository repository;

  private final ArticleResponseFactory responseFactory;

  public FetchArticlesFromUrlService(
  ...
  public Collection<ArticleResponse> execute(FetchArticlesFromUrlRequest request) {
    return repository
        .fetchFromUrl(request.getUrl())
        .stream()
        .map(responseFactory::articleResponse)
        .collect(toList());
  }
}
